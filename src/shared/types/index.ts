...
/** Matches `FIELD_TYPES` in the backend's `Form.ts`. */
...
export type FormField = {
  /** Stable machine key. Immutable once the form has submissions — see `lockedFieldKeys`. */
  key: string;
  label: string;
  type: FormFieldType;
  required: boolean;
  ...
  maxLength: number;
  order: number;
  /** Ends a page here in the hosted multi-page renderer. */
...
export type FormSettings = {
  ...
  /** Days to keep a submission. `null`/undefined means keep until deleted. */
  retentionDays?: number | null;
  /** Preset slug from `ThemePreset[]`. Defaults to `"default"` server-side. */
  theme?: string;
  /** Per-form overrides layered on top of the chosen preset. */
  themeOverrides?: {
    primaryColor?: string;
...
/** One entry from the `GET /api/public/forms/themes` catalogue. */
