...
export function HeroMock() {
  return (
    <svg viewBox="0 0 520 340" className="hero-mock" role="img" aria-label="Analytics dashboard preview">
      ...
      <circle cx="54" cy="22" r="4" fill="var(--accent)" />
      <line x1="0" y1="44" x2="520" y2="44" stroke="#2a2c36" />

      {[
        { x: 20, label: "Visitors", val: "12.8k", c: "var(--accent)" },
        { x: 190, label: "Pageviews", val: "48.2k", c: "#f3f4f6" },
        { x: 360, label: "Live", val: "326", c: "var(--accent)", live: true },
      ].map((k) => (
        <g key={k.label} transform={`translate(${k.x},62)`}>
          ...
          {k.live && <circle cx="118" cy="18" r="4" fill="var(--accent)" className="pulse-dot" />}
        </g>
      ))}
      ...
      <g transform="translate(20,278)">
        {[70, 55, 40, 28].map((w, i) => (
          <rect key={i} y={i * 15} width={w * 4} height="9" rx="3" fill="var(--accent)" opacity={0.8 - i * 0.15} />
        ))}
      </g>
    </svg>
  );
    ...
    <svg viewBox="0 0 260 150" width="260" height="150" role="img" aria-label="Analytics illustration">
      <defs>
        ...
        <linearGradient id="aa-line" x1="0" y1="0" x2="1" y2="0">
          <stop stopColor="#22d3ee" /><stop offset="1" stopColor="var(--accent)" />
        </linearGradient>
      </defs>
      {/* baseline */}
      ...
      {[[40,78],[91,52],[129,66],[167,34],[205,56]].map(([x,y],i)=>(
        <circle key={i} cx={x} cy={y} r="4" fill="#fff" stroke="var(--accent-2)" strokeWidth="2" opacity="0">
