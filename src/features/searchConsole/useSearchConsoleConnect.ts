...
const REASON_TEXT: Record<string, string> = {
  ...
  scope_declined:
    "Search visibility access was not granted. Tick the Search Console permission on Google's consent screen.",
...
export function useSearchConsoleConnect(workspaceId: string | undefined, onConnected?: () => void) {
  return useOAuthPopup({
    ...
    successMessage: "Search visibility connected",
    cancelledMessage: "Search visibility connection cancelled",
    fallbackError: "Could not connect Search visibility. Please try again.",
    onDone: onConnected,
  });
}
